...
/// Menu bar label: provider glyph only, filled bottom-to-top by the selected
/// metric's usage fraction (100% = fully filled glyph).
struct MenuBarLabel: View {
    ...
    var body: some View {
        FilledProviderIcon(kind: store.snapshot?.providerKind ?? .generic,
                           fraction: store.snapshot.flatMap { $0.fraction(for: metric) },
                           size: 18)
    }
}
...
struct FilledProviderIcon: View {
    ...
    var body: some View {
        ZStack {
            ProviderIcon(kind: kind, size: size, color: .primary)
                .opacity(0.3)
